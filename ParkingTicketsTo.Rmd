...
```{r}
library(dplyr)
date_group <- group_by(parking_df, date_of_infraction)
parking_df_day <- summarize(date_group, 
                            total_day = sum(set_fine_amount), 
                            n = n())
head(arrange(parking_df_day, desc(n)), n = 5)
tail(arrange(parking_df_day, desc(n)), n = 5)
```
...
parking_df_weekday <- summarize(weekday_group, 
                                weekday_sum = sum(set_fine_amount), 
                                n = n())
...
ggplot(aes(x = day_of_week, y = n), data = parking_df_weekday) +
...
parking_df_infr <- summarize(infraction_group, 
                           total = sum(set_fine_amount), 
                           n = n())
```

```{r}
parking_df_infr <- head(arrange(parking_df_infr,desc(n)), n = 10)
parking_df_infr
```
...
```{r}
library(ggthemes)
ggplot(aes(x = reorder(infraction_description, n), y = n), data = parking_df_infr) +
  geom_bar(stat = 'identity') +
  theme_tufte() +
...
parking_df_lo <- summarize(location_group, 
                           total = sum(set_fine_amount), 
                           n = n())

parking_df_lo <- head(arrange(parking_df_lo,desc(n)), n = 10)

ggplot(aes(x = reorder(location2, n), y = n), data = parking_df_lo) +
  geom_bar(stat = 'identity') +
  theme_tufte() +
  theme(axis.text = element_text(size = 10, face = 'bold')) +
  coord_flip() +
...
parking_df_infr_1 <- parking_df %>%
  ...
  group_by(infraction_description, date_of_infraction) %>%
  summarize(n = n(), total = sum(set_fine_amount))
```

```{r}
ggplot(aes(x = date_of_infraction, y = n, color = infraction_description), data = parking_df_infr_1) +
...
parking_df_week_time <- summarize(week_time_group, 
                             time_sum = sum(set_fine_amount), 
                             n = n())
```
...
ggplot(aes(x = time_of_infraction, y = n, color = day_of_week), data = parking_df_week_time) +
...
parking_df_top_10 <- parking_df %>%
  ...
  group_by(infraction_description, day_of_week, time_of_infraction) %>%
  summarize(n = n(), total = sum(set_fine_amount))
```
...
ggplot(aes(x = time_of_infraction, y = n, color = day_of_week), data = parking_df_top_10) +
  geom_line(size = 1.5) +
  geom_point(size = 0.5) +
  xlab('Hour(24 hour clock)') +
  ylab('Number of Infractions') +
